...
/* ── Single bar color ── */
const BLUE = { hex: "#4A8FE8", rgb: "74,143,232" };
...
function riskBadge(v: number): { text: string; color: string; bg: string; border: string } {
  if (v > 65)  return { text: "LOW RISK",    color: "#4ADE80", bg: "rgba(74,222,128,0.08)",  border: "rgba(74,222,128,0.2)"  };
  if (v >= 40) return { text: "MEDIUM RISK", color: "#FACC15", bg: "rgba(250,204,21,0.08)",  border: "rgba(250,204,21,0.2)"  };
  return        { text: "HIGH RISK",          color: "#F87171", bg: "rgba(248,113,113,0.08)", border: "rgba(248,113,113,0.2)" };
}
...
/* ── Frame thumbnails — light-mode mockups ── */
function FrameA({ variant }: { variant: number }) {
  ...
  return (
    <svg viewBox="0 0 80 50" width="100%" height="100%" preserveAspectRatio="none">
      <rect width="80" height="50" fill="#EFEFEF" />
      <rect width="80" height="7" fill="#CCCCCC" />
      <rect y="7" width="18" height="43" fill="#DDDDDD" />
      <rect x="22" y={s} width="24" height="10" rx="1" fill="#DDDDDD" />
      <rect x="50" y={s + 4} width="20" height="8" rx="1" fill="#DDDDDD" />
      <rect x={80 - w - 4} y="2" width={w} height="3" rx="1" fill="#4A8FE8" />
    </svg>
  );
}
function FrameB({ variant }: { variant: number }) {
  ...
  return (
    <svg viewBox="0 0 80 50" width="100%" height="100%" preserveAspectRatio="none">
      <rect width="80" height="50" fill="#EFEFEF" />
      <rect width="80" height="8" fill="#CCCCCC" />
      {[0, 1, 2].map(c => (
        <g key={c}>
          <rect x={4 + c * 26} y="12" width="22" height="16" rx="1" fill="#DDDDDD" stroke="#CCCCCC" strokeWidth="0.5" />
          <rect x={4 + c * 26} y="12" width="22" height="3" rx="1" fill={c === hi ? "#4A8FE8" : "#CCCCCC"} />
          <rect x={6 + c * 26} y="18" width="14" height="2" rx="0.5" fill="#CCCCCC" />
          <rect x={6 + c * 26} y="22" width="10" height="2" rx="0.5" fill="#CCCCCC" />
        </g>
      ))}
    </svg>
  );
...
function ReportModal({ content, onClose }: { content: string; onClose: () => void }) {
  return (
    <div
      style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(4px)", WebkitBackdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 24, boxSizing: "border-box" }}
      ...
      <div className="glass-card" style={{ width: "100%", maxWidth: 680, maxHeight: "80vh", display: "flex", flexDirection: "column", padding: 0, overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 24px", borderBottom: "1px solid rgba(255,255,255,0.06)", flexShrink: 0 }}>
          ...
          <button onClick={onClose} style={{ background: "transparent", border: "none", cursor: "pointer", color: "#666", fontSize: 18, lineHeight: 1, padding: "2px 6px", borderRadius: 2 }}
            onMouseEnter={e => { e.currentTarget.style.color = "#fff"; }} onMouseLeave={e => { e.currentTarget.style.color = "#666"; }}>x</button>
        </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
        ...
        <div style={{ height: "65%", display: "flex", flexDirection: "row", overflow: "hidden" }}>
          ...
          <div className="region-bars-scroll" style={{ width: "60%", height: "100%", overflowY: "auto", padding: "20px 20px 20px 24px", boxSizing: "border-box" }}>
            <p style={{ fontSize: 10, fontFamily: "monospace", color: "#777777", letterSpacing: "0.15em", textTransform: "uppercase", margin: 0, marginBottom: 16 }}>
            ...
            {REGIONS.map((r, i) => {
              ...
              return (
                <div key={r.scientific} style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 4, padding: 16, marginBottom: 8 }}>
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                    <div>
                      <span style={{ fontSize: 15, fontWeight: 500, color: "#FFFFFF", display: "block", lineHeight: 1.3 }}>{r.plain}</span>
                      <span style={{ fontSize: 10, color: "#666666", fontFamily: "monospace", lineHeight: 1.3 }}>{r.scientific}</span>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0, marginLeft: 12 }}>
                      <span style={{
                        fontSize: 9, fontFamily: "monospace", borderRadius: 2,
                        padding: "3px 8px", letterSpacing: "0.1em",
                        background: risk.bg,
                        border: `1px solid ${risk.border}`,
                        color: risk.color,
                      }}>{risk.text}</span>
                      <span style={{ fontSize: 13, fontFamily: "monospace", color: BLUE.hex }}>{v}%</span>
                    </div>
                  </div>
                  {/* Bar */}
                  <div style={{ position: "relative", height: 3, marginTop: 10, borderRadius: 2 }}>
                    ...
                    <div style={{
                      ...
                      background: BLUE.hex,
                      borderRadius: 2,
                      boxShadow: `0 0 8px rgba(${BLUE.rgb},0.5), 0 0 2px rgba(${BLUE.rgb},0.8)`,
                      transition: "width 0.15s ease",
                    }} />
                  </div>
                  <p style={{ margin: 0, marginTop: 10, fontSize: 13, color: "#AAAAAA", lineHeight: 1.7 }}>{churn[i]}</p>
                </div>
              );
            })}
          </div>
          ...
          <div style={{ width: "40%", height: "100%", overflow: "hidden", borderLeft: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.03)", display: "flex", flexDirection: "column", padding: "20px 24px", gap: 20, boxSizing: "border-box" }}>

            {/* Cognitive load */}
            <div style={{ flexShrink: 0 }}>
              <p style={{ fontSize: 10, fontFamily: "monospace", color: "#888888", letterSpacing: "0.15em", textTransform: "uppercase", margin: 0, marginBottom: 14 }}>Cognitive Load Score</p>
              <div style={{ display: "flex", alignItems: "baseline", gap: 28 }}>
                <div style={{ display: "flex", alignItems: "baseline" }}>
                  <span style={{ fontSize: 11, fontFamily: "monospace", color: "#777777", marginRight: 6 }}>A</span>
                  <span style={{ fontSize: 52, fontWeight: 300, lineHeight: 1, color: "#FFFFFF" }}>{COG_A}</span>
                </div>
                <div style={{ display: "flex", alignItems: "baseline" }}>
                  <span style={{ fontSize: 11, fontFamily: "monospace", color: "#777777", marginRight: 6 }}>B</span>
                  <span style={{ fontSize: 52, fontWeight: 300, lineHeight: 1, color: "#FFFFFF" }}>{COG_B}</span>
                </div>
              </div>
            </div>
            ...
            {/* Audit confidence */}
            <div style={{ flexShrink: 0 }}>
              <p style={{ fontSize: 10, fontFamily: "monospace", color: "#888888", letterSpacing: "0.15em", textTransform: "uppercase", margin: 0, marginBottom: 10 }}>Audit Confidence</p>
              <p style={{ fontSize: 20, fontWeight: 400, color: "#FFFFFF", margin: 0 }}>High confidence</p>
              <p style={{ fontSize: 13, color: "#999999", lineHeight: 1.6, margin: 0, marginTop: 8 }}>
                Based on 752 Meta FAIR baseline scans. Neural response patterns validated across a diverse demographic sample.
              </p>
            </div>
            ...
            <div style={{ flexShrink: 0 }}>
              {[["Audit date", "April 1 2026"], ["Design A", "upload_a.mp4"], ["Design B", "upload_b.mp4"], ["Duration", "30s, 8 frames"]].map(([l, v]) => (
                <div key={l} style={{ display: "flex", gap: 16, lineHeight: 1.8 }}>
                  <span style={{ fontSize: 10, fontFamily: "monospace", color: "#666666", width: 80, flexShrink: 0 }}>{l}</span>
                  <span style={{ fontSize: 11, fontFamily: "monospace", color: "#AAAAAA" }}>{v}</span>
                </div>
              ))}
            </div>
            ...
            <div style={{ marginTop: "auto", flexShrink: 0, display: "flex", justifyContent: "flex-end" }}>
              <button
                ...
                style={{
                  padding: "12px 20px",
                  background: btnHover ? "rgba(74,143,232,0.08)" : "transparent",
                  border: btnHover ? "1px solid rgba(74,143,232,0.8)" : "1px solid rgba(74,143,232,0.5)",
                  borderRadius: 4, color: "#4A8FE8",
        ...
        <div style={{ height: "35%", borderTop: "1px solid rgba(255,255,255,0.08)", background: "rgba(255,255,255,0.04)", padding: "14px 20px", display: "flex", flexDirection: "column", gap: 10, overflow: "hidden" }}>

          {/* Label row */}
          <div style={{ display: "flex", justifyContent: "space-between", flexShrink: 0 }}>
            <span style={{ fontSize: 10, fontFamily: "monospace", color: "#888888", letterSpacing: "0.14em" }}>
              DESIGN {mode.toUpperCase()} PLAYBACK
            </span>
            <span style={{ fontSize: 11, fontFamily: "monospace", color: "#888888" }}>
          ...
          <div style={{ display: "flex", gap: 8, flex: 1, minHeight: 0 }}>
            {Array.from({ length: 8 }, (_, i) => {
              ...
              const isHovered = hoveredFrame === i && !isActive;
              return (
                <div
                  key={i}
                  onClick={() => setCurrentTime(i * 4)}
                  ...
                  style={{
                    ...
                    border: isActive ? `1px solid ${bc}` : "none",
                    borderTop: isActive ? undefined : `2px solid ${bc}`,
                    boxShadow: isActive
                      ? `0 0 12px ${hexToRgba(bc, 0.3)}`
          ...
          <div
            ref={barRef}
            style={{ position: "relative", height: 14, flexShrink: 0, cursor: "pointer" }}
          ...
          >
            {/* Track */}
            <div style={{ position: "absolute", left: 0, right: 0, top: 6, height: 2, background: "rgba(255,255,255,0.1)", borderRadius: 2 }} />
            {/* Fill */}
            <div style={{ position: "absolute", left: 0, top: 6, height: 2, width: `${fillPct}%`, background: "#4A8FE8", opacity: 0.8, borderRadius: 2, transition: dragging.current ? "none" : "width 0.15s ease" }} />
            {/* Scrubber dot */}
            <div style={{
              ...
              boxShadow: "0 0 8px rgba(74,143,232,0.6)",
          ...
          <div style={{ display: "flex", justifyContent: "space-between", flexShrink: 0 }}>
            {TIMESTAMPS.map((t, i) => (
              <span key={i} style={{ fontSize: 10, fontFamily: "monospace", color: "#777777", width: "calc(100%/8)", textAlign: "center" }}>{t}</span>
            ))}
          </div>
        </div>
      </div>
